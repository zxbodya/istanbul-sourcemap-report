...
describe('remap', () => {
  it('remapping', () => {
    ...
    expect(coverage.store.map[r('spec/fixtures/basic.ts')]).toBeTruthy();
    expect(Object.keys(coverage.store.map).length).toEqual(1);

    const map = JSON.parse(coverage.store.map[r('spec/fixtures/basic.ts')]);
    expect(map.path, r('spec/fixtures/basic.ts'));
  ...
  it('base64 source map with sources', () => {
    ...
    expect(store.map[r('spec/fixtures/inlinesource.ts')]).toBeTruthy();
  });

  it('coverage includes code', () => {
    const coverage = remap([loadCoverage('spec/fixtures/coverage-code.json')]);
    expect(coverage instanceof Collector).toBeTruthy();
    expect(coverage.store.map[r('spec/fixtures/inlinesource.ts')]).toBeTruthy();

    const map = JSON.parse(coverage.store.map[r('spec/fixtures/inlinesource.ts')]);
  ...
  it('unicode in map', () => {
    ...
    expect(coverage.store.map[r('spec/fixtures/unicode.ts')]).toBeTruthy();
    expect(Object.keys(coverage.store.map).length).toEqual(1);
  });
  ...
  it('skip in source map', () => {
    ...
    const coverageData = JSON.parse(coverage.store.map[r('spec/fixtures/basic.ts')]);
  ...
  it('non transpiled coverage', () => {
    ...
    const coverageData = JSON.parse(coverage.store.map[r('spec/fixtures/foo.js')]);
