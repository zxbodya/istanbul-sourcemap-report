...
describe('remap', () => {
  ...
  it('base64 source map with sources', () => {
    ...
    expect(store.map['inlinesource.ts']).toBeTruthy();
  });

  it('coverage includes code', () => {
    const coverage = remap(loadCoverage('spec/fixtures/coverage-code.json'));
    expect(coverage instanceof Collector).toBeTruthy();
    expect(coverage.store.map['inlinesource.ts']).toBeTruthy();

    const map = JSON.parse(coverage.store.map['inlinesource.ts']);
  ...
  it('basePath', () => {
    ...
    expect(coverage.store.map['foo/bar/basic.ts'], 'The base path provided should have been used');
    expect(Object.keys(coverage.store.map).length).toEqual(1);
    const map = JSON.parse(coverage.store.map['foo/bar/basic.ts']);
    expect(map.path).toEqual('foo/bar/basic.ts');
  });
